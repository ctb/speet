...
def sketch(args):
    ...
    mh.save(args.output)
    print(
        "read {}, hashed to {}, saved into {}".format(len(text), len(mh), args.output)
    )
...
def fragment_query(args):
    ...
    print(
        "got {} fragments of size {} from {}".format(len(fragments), chunk, args.query)
    )
...
def main(argv=None):
    ...
    fragment_query_p.add_argument(
        "--fragment-size", default=5000, help="size (in char) of query fragments"
    )
    fragment_query_p.add_argument(
        "--threshold", default=0.2, help="fraction of query fragment required"
    )
